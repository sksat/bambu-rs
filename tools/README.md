...
Two exceptions. `trust_relay_in_studio.sh` changes something: it is here because
it belongs to the same job — making a real client talk to this crate — and
because a script that edits an installed application's trust store should be
reviewable rather than pasted from a chat. And `doom/` is not an observation
script at all; see its own README.
