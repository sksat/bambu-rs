//! A client's command, read as a key press.
//!
//! The half of [`super`] that faces the client: a jog on Y is a step forward,
//! the home button is the trigger, the chamber light opens doors.
//!
//! Pure, and the clock belongs to the caller — every entry point takes a
//! monotonic millisecond stamp — so the hold logic is driven from a test
//! without waiting for anything.
//!
//! **A press has a duration**, which is the one thing that makes this more
//! than a lookup table. DOOM moves you for as long as a key is down, so a
//! button that is only ever tapped would leave the player twitching on the
//! spot. The duration comes from the jog itself: the millimetres the client
//! asked the machine to move become the milliseconds the key is held, so
//! Studio's 1 mm and 10 mm buttons are a nudge and a step.
