---
name: bambu-slice
description: Slice an STL/STEP/3MF for the Bambu Lab A1 mini with OrcaSlicer (headless CLI) and optionally upload + print it via the bambu CLI or the `bambu serve` HTTP API. Use whenever someone wants to prepare a model for printing on the A1 mini — "slice this for my A1", "get this STL ready to print", choosing a layer height/filament, or generating a .gcode.3mf. Reach for this even if they don't name OrcaSlicer, because the CLI has a sharp inheritance trap (non-default settings silently revert to defaults) that this skill handles.
metadata:
  type: reference
---
